...
class NeRF(nn.Module):
    ...
    def __init__(self, L_pos=10, L_dir=4, hidden_dim=256):
        ...
        self.block1=nn.Sequential(nn.Linear(L_pos*6+3, hidden_dim),nn.ReLU(),
                                  nn.Linear(hidden_dim, hidden_dim),nn.ReLU(),
                                  nn.Linear(hidden_dim, hidden_dim),nn.ReLU(),
                                  nn.Linear(hidden_dim, hidden_dim),nn.ReLU(),
                                  nn.Linear(hidden_dim, hidden_dim),nn.ReLU())
        
        self.block2=nn.Sequential(nn.Linear(hidden_dim+L_pos*6+3, hidden_dim),nn.ReLU(),
                                  nn.Linear(hidden_dim, hidden_dim),nn.ReLU(),
                                  nn.Linear(hidden_dim, hidden_dim),nn.ReLU(),
                                  nn.Linear(hidden_dim, hidden_dim+1),)
        
        self.rgb_head=nn.Sequential(nn.Linear(hidden_dim+L_dir*6+3, hidden_dim//2),nn.ReLU(),
    ...
    def forward(self,xyz,d):
        x_emb= self.positional_encoding(xyz,self.L_pos)
        d_emb= self.positional_encoding(d,self.L_dir)
